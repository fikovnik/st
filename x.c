...
void xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len,
  ...
  if (base.mode & ATTR_UNDERLINE) {
    if (base.ustyle == 0) {
      ...
      XDrawPoints(xw.dpy, XftDrawDrawable(xw.draw), gc, points, width,
                  CoordModeOrigin);
...
void generatewave(int x, int y, int width, XPoint **points) {
  ...
  if (width > 0) {
    ...
    *points = ps;
    ...
    if (dc.state.waveline == y && dc.state.wavestart <= x &&
        dc.state.waveend >= x) {
...
void kpress(XEvent *ev) {
  ...
  for (bp = shortcuts; bp < shortcuts + LEN(shortcuts); bp++) {
    if (ksym == bp->keysym && match(bp->mod, e->state) &&
        (!isaltscreenon() || bp->altscreenmode)) {
      bp->func(&(bp->arg));
      return;
    }
  }
